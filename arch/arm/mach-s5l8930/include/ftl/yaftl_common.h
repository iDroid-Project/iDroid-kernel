...
extern struct apple_vfl* vfl;

/* Functions */

extern void YAFTL_setupCleanSpare(SpareData* _spare_ptr);

extern int YAFTL_readMultiPages(uint32_t* pagesArray, uint32_t nPages, uint8_t* dataBuffer, SpareData* metaBuffers, uint32_t disableAES, uint32_t scrub);

extern int YAFTL_writeMultiPages(uint32_t _block, uint32_t _start, size_t _numPages, uint8_t* _pBuf, SpareData* _pSpare, uint8_t _scrub);

extern void YAFTL_allocateNewBlock(uint8_t isUserBlock);

extern error_t YAFTL_closeLatestBlock(uint8_t isUserBlock);

extern uint32_t* YAFTL_allocBTOC(uint32_t _arg0);

extern uint32_t* YAFTL_getBTOC(uint32_t _block);

extern error_t YAFTL_readBTOCPages(uint32_t offset, uint32_t *data, SpareData *spare, uint8_t disable_aes, uint8_t scrub, uint32_t max);

extern error_t YAFTL_readPage(uint32_t _page, uint8_t* _data_ptr, SpareData* _spare_ptr, uint32_t _disable_aes, uint32_t _empty_ok, uint32_t _scrub);
...
extern void YAFTL_copyBTOC(uint32_t* dest, uint32_t* src, uint32_t maxVal);
