...
static void deallocBTOC(uint32_t* btoc)
{
	...
	panic("YAFTL: couldn't deallocate BTOC %p\r\n", btoc);
}
...
int YAFTL_readMultiPages(uint32_t* pagesArray, uint32_t nPages, uint8_t* dataBuffer, SpareData* metaBuffers, uint32_t disableAES, uint32_t scrub)
{
	...
	for (i = 0; pagesToRead > sGeometry.pagesPerSublk; i++) {
		...
		if (ret) {
			...
			if (!succeeded) {
				printk(KERN_INFO "yaftl: _readMultiPages: We weren't able to overcome read failure.\r\n");
				return 0;
			}
...
int YAFTL_writeMultiPages(uint32_t _block, uint32_t _start, size_t _numPages, uint8_t* _pBuf, SpareData* _pSpare, uint8_t _scrub)
{
	...
	for (i = 0; i < _numPages; ++i) {
		...
		if (FAILED(result)) {
			printk(KERN_INFO "YAFTL: writeMultiPages got write failure: block %d, "
...
void YAFTL_allocateNewBlock(uint8_t isUserBlock)
{
		...
		if (unkn5 != 2 && unkn5 != 5) {
			...
			if (sInfo.blockArray[currBlk].eraseCount < minEraseCnt) {
				if (sInfo.blockArray[currBlk].validPagesINo) {
					panic("YAFTL: YAFTL_allocateNewBlock found an empty block"
	...
	if (bestBlk == 0xFFFFFFFF)
		panic("YAFTL: YAFTL_allocateNewBlock is out of blocks\r\n");
...
uint32_t* YAFTL_allocBTOC(uint32_t _block)
{
	...
	if (found == -1)
		panic("yaftl: couldn't allocate a BTOC\r\n");
...
error_t YAFTL_readPage(uint32_t _page, uint8_t* _data_ptr, SpareData* _spare_ptr, uint32_t _disable_aes, uint32_t _empty_ok, uint32_t _scrub)
{
	...
	result = vfl->read_single_page(vfl, _page, _data_ptr, meta_ptr, _empty_ok,
		&refreshPage);

	if (FAILED(result)) {
		printk(KERN_INFO "YAFTL_readPage: We got read failure: page %d, block %d, block status %d, scrub %d.\r\n",
			_page,
			block,
...
uint32_t YAFTL_writePage(uint32_t _page, uint8_t* _data_ptr, SpareData* _spare_ptr)
{
	...
	result = vfl->write_single_page(vfl, _page, _data_ptr, (uint8_t*)_spare_ptr, 1);

	if (FAILED(result)) {
		printk(KERN_INFO "YAFTL_writePage: We got write failure: page %d, block %d, block status %d.\r\n",
			_page,
			block,
			sInfo.blockArray[block].status);
...
error_t YAFTL_writeIndexTOC()
{
	...
	printk(KERN_INFO "WARNING - You NEED to call ftl_flush before rebooting. - WARNING\r\n");

	YAFTL_setupCleanSpare(sInfo.spareBuffer7);

	// Try to write the new index TOC.
	result = vfl->write_single_page(vfl,
	...
	if (FAILED(result)) {
		...
		vfl->erase_single_block(vfl, sInfo.FTLCtrlBlock[sInfo.selCtrlBlockIndex], 1);
	...
	if (bestDirty.idx != 0xFFFF) {
		...
		if (oldIndexPage != 0xFFFFFFFF) {
			...
			if (sInfo.blockArray[oldIndexBlock].validPagesINo == 0) {
				panic("YAFTL: clearEntryInCache tried to invalidate an "
		...
		if (result) {
			panic("YAFTL: clearEntryInCache failed to write dirty index"
